...
namespace Mp3Mangler
{
	public class Mp3Processor
	{
		...
		/// <summary>
		/// Cleans up MP3 tags for all files in the passed <paramref name="directory"/>
		/// </summary>
		/// <param name="directory">Folder to clean</param>
		public void CleanFiles(string directory)
		{
			var files = _reader.GetFileNamesFromDirectory(directory);

			foreach (var file in files)
			{
				TagLib.File mp3File = _reader.GetFile(file);
				bool isDirty = ProcessMp3File(ref mp3File);
		...
		private bool isPartNumeric(string text)
		{
